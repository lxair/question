...
class TransGANModel(Model):
    ...
    def train_epoch_gan(self, data_loader,epoch,gen_avg_param,writer):
        ...
        for iter_idx,sequence_batch in enumerate(tqdm(data_loader)):
            ...
            # 取数据
            real_sequence = sequence_batch[:, :, 0:-1].to(self.device) # 取前三个时间特征。 [256,30,158]
            ...
            real_sequence = real_sequence.reshape(real_sequence.shape[0],real_sequence.shape[2], 1 ,real_sequence.shape[1],) #把[256,30,158]变为[256,158,1,30]
            real_data_list.append(real_sequence)
            
            # 真实的数据放到判别器
            discriminator_output_real = self.discriminator(real_sequence)# 将真实的值传到鉴别器中，先学习真实的数据。discriminator_output_real.shape(256,1)
...
class PatchEmbedding_Linear(nn.Module):
    ...
    def __init__(self, in_channels = 158, patch_size = 15, emb_size = 15, seq_length = 30):
        ...
        self.projection = nn.Sequential(
            Rearrange('b c (h s1) (w s2) -> b (h w) (s1 s2 c)',s1 = 1, s2 = patch_size), # 这里(h h1) (w w1)就相当于h与w变为原来的1/h1,1/w1倍
    ...
    def forward(self, x: Tensor) -> Tensor:  # (64,158,1,30)
        ...
        x = torch.cat([cls_tokens, x], dim=1)  # x又变成了(512,2,15)
        # position
        x += self.positions.expand_as(x)
        return x         #x.shape(512,3,15)
